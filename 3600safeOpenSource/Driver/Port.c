...
PVOID EnumPortInformation(OUT PULONG  OutLength,IN USHORT  PortType)
{
	...
	RIoGetRelatedDeviceObject = ReLoadNtosCALL(L"IoGetRelatedDeviceObject",SystemKernelModuleBase,ImageModuleBase);
...
{ 
	...
	if (pTcpBuf != NULL)
	{
		...
		for(;pTcpInfo->dwOwningPid; pTcpInfo++)
		{
			...
			{
				if (!GetProcessImagePath(pTcpInfo->dwOwningPid,TCPUDPInfo->TCPUDP[i].lpwzFullPath))
				{
