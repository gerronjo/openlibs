...
NTSTATUS GetCreateProcessNotify(PSYSTEM_NOTIFY SystemNotify)
{
	PEX_FAST_REF Ref;
...
*/
		for(i=Base;i<i+SizeOfProc(Base);i++)
		{
			if(*i == 0xe8)
			{
				...
				if (MmIsAddressValid(ulPspSetCreateProcessNotifyRoutine))
				{
...
NTSTATUS GetCreateThreadNotify(PSYSTEM_NOTIFY SystemNotify)
{
	PEX_FAST_REF Ref;
...
NTSTATUS GetLoadImageNotify(PSYSTEM_NOTIFY SystemNotify)
{
	PEX_FAST_REF Ref;
...
NTSTATUS GetRegisterNotify(PSYSTEM_NOTIFY SystemNotify)
{
	PEX_FAST_REF Ref;
...
*/
NTSTATUS GetBugCheckCallBack(PSYSTEM_NOTIFY SystemNotify)
{
...
NTSTATUS GetBugCheckReasonCallBack(PSYSTEM_NOTIFY SystemNotify)
{
...
NTSTATUS GetFileSystemNotifyCallBack(PSYSTEM_NOTIFY SystemNotify)
{
	PLIST_ENTRY pListEntry;
	PLIST_ENTRY pListNext;
...
NTSTATUS GetShutDownCallBack(PSYSTEM_NOTIFY SystemNotify)
{
...
// 		IopFsNotifyChangeQueueHead=Addr;
		for (i=(ULONG)Addr;i < i+SizeOfProc(Addr);i++)
		{
			if (*i == 0xbe)
			{
				...
				if (MmIsAddressValid(IopFsNotifyChangeQueueHead))
				{
					return;
				}
			}
	...
	case WINDOWS_VERSION_7:
		for (i=(ULONG)Addr;i < i+SizeOfProc(Addr);i++)
		{
			if (*i == 0xe8)
			{
				...
				if (MmIsAddressValid(ulIoRegisterFsRegistrationChangeMountAware))
				{
					if (DebugOn)
...
// 	//-----------------------------------------------------------------------//注册表
// 	GetRegisterNotify(SystemNotify);
// 	//-----------------------------------------------------------------------//BugCheck
...
VOID RemoveNotifyRoutine(ULONG ImageNotify,int Type)
{
	...
	case 1:
		...
		RPsSetCreateProcessNotifyRoutine = ReLoadNtosCALL(L"PsSetCreateProcessNotifyRoutine",SystemKernelModuleBase,ImageModuleBase);
	...
	case 2:
		...
		RPsRemoveCreateThreadNotifyRoutine = ReLoadNtosCALL(L"PsRemoveCreateThreadNotifyRoutine",SystemKernelModuleBase,ImageModuleBase);
	...
	case 3:
		//LoadImage
		//PsRemoveLoadImageNotifyRoutine(ImageNotify);
		RPsRemoveLoadImageNotifyRoutine = ReLoadNtosCALL(L"PsRemoveLoadImageNotifyRoutine",SystemKernelModuleBase,ImageModuleBase);
	...
	case 4:
		...
		RKeDeregisterBugCheckCallback = ReLoadNtosCALL(L"KeDeregisterBugCheckCallback",SystemKernelModuleBase,ImageModuleBase);
	...
	case 5:
		...
		RKeDeregisterBugCheckReasonCallback = ReLoadNtosCALL(L"KeDeregisterBugCheckReasonCallback",SystemKernelModuleBase,ImageModuleBase);
	...
	case 6:
		...
		RIoUnregisterShutdownNotification = ReLoadNtosCALL(L"IoUnregisterShutdownNotification",SystemKernelModuleBase,ImageModuleBase);
	...
	case 7:
		...
		RSeUnregisterLogonSessionTerminatedRoutine = ReLoadNtosCALL(L"SeUnregisterLogonSessionTerminatedRoutine",SystemKernelModuleBase,ImageModuleBase);
