...
NTSTATUS QueryServicesRegistry(PSERVICESREGISTRY ServicesRegistry)
{
	...
	RZwOpenKey = ReLoadNtosCALL(L"ZwOpenKey",SystemKernelModuleBase,ImageModuleBase);
	...
	RZwEnumerateKey = ReLoadNtosCALL(L"ZwEnumerateKey",SystemKernelModuleBase,ImageModuleBase);
	RZwQueryValueKey = ReLoadNtosCALL(L"ZwQueryValueKey",SystemKernelModuleBase,ImageModuleBase);
			...
			if ((Status = RZwQueryValueKey(SubKeyHandle, &ImagePath, KeyValuePartialInformation, NULL, 0, &ResultLength)) != STATUS_OBJECT_NAME_NOT_FOUND)
			{
				...
				if (KeyValueInformation)
				{
					...
					if (NT_SUCCESS(Status))
					{
						...
						if (RMmIsAddressValid(KeyValueInformation->Data) &&
							...
							DataSize < 1024)
						{
							...
							if (RRtlCompareUnicodeString(&UnicodeRegLocation,&UnicodeComPareKey,TRUE) == NULL)  //不区分大小写
							{
									...
									ulLogCount < 1000)   //记录超过1000条，则不记录。
								{
									...
									LogDefenseInfo->LogDefense[ulLogCount].ulInheritedFromProcessId = GetInheritedProcessPid(RPsGetCurrentProcessId());
...
_FunctionRet:
	...
	if (RMmIsAddressValid(OriginalServiceDescriptorTable->ServiceTable[ZwSetValueKeyIndex]))
