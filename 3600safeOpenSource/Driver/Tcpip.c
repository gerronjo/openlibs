...
VOID FixFixTcp(PTCPDISPATCHBAKUP TcpDispatchBakUp,PDRIVER_OBJECT PTcpDriverObject,int i,ULONG ulReal_Dispatch,WCHAR *lpwzDispatchName,ULONG Dispatch)
{
	...
	if (ulCurrentTcpDispatch == ulReal_Dispatch)
	{
			...
			memcmp(ulReal_Dispatch,ulReloadTcpDispatch,GetFunctionCodeSize(ulReal_Dispatch)) != NULL)
		{
			...
			if (!RMmIsAddressValid(ulCurrentTcpDispatch))
			{
...
_FunRet:
	...
	{
		strcat(TcpDispatchBakUp->TcpDispatch[i].lpszBaseModule,"Unknown3");
	}
...
VOID SetTcpHook(ULONG ulNumber,ULONG ulRealDispatch)
{
	...
	if (RMmIsAddressValid(PTcpDriverObjectBakup))
	{
...
NTSTATUS ReLoadTcpip(PDRIVER_OBJECT DriverObject,PTCPDISPATCHBAKUP TcpDispatchBakUp,int Type)
{
	...
	if (GetDriverObject(L"\\Driver\\Tcpip",&PTcpipDriverObject) == STATUS_SUCCESS)
	{
		...
		{
			if (DebugOn)
				KdPrint(("WINDOWS_VERSION_2K3_SP1_SP2:%08X\n",ulTcpipModuleBase));
			goto _FunRet;
		}
		//reload
		if (PeLoad(
...
_FunRet:
		if (!bIsReLoadSuccess)
		{
			//KdPrint(("MapFileAsSection\n"));

			RtlInitUnicodeString(&UnicodeModule,L"\\SystemRoot\\system32\\drivers\\tcpip.sys");
			hSection = MapFileAsSection(&UnicodeModule,&ulModuleBase);
			if (hSection)
			{
				ulReLoadTcpipModuleBase = ulModuleBase;
				//KdPrint(("ulModuleBase:%08X\n",ulModuleBase));
				ZwClose(hSection);
			}else
				KdPrint(("MapFileAsSection false\n"));
		}
...
// 				break;
			...
			ulReal_TCP_IRP_MJ_CREATE = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_CREATE_NAMED_PIPE = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_CLOSE = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_READ = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_WRITE = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_QUERY_INFORMATION = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_SET_INFORMATION = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_QUERY_EA = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_SET_EA = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_FLUSH_BUFFERS = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_QUERY_VOLUME_INFORMATION = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_SET_VOLUME_INFORMATION = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_DIRECTORY_CONTROL = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_FILE_SYSTEM_CONTROL = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_DEVICE_CONTROL = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_SHUTDOWN = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_LOCK_CONTROL = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_CLEANUP = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_CREATE_MAILSLOT = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_QUERY_SECURITY = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_SET_SECURITY = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_POWER = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_SYSTEM_CONTROL = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_DEVICE_CHANGE = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_QUERY_QUOTA = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_SET_QUOTA = *(PULONG)(ulAddress+1) + ulOffset;
			ulReal_TCP_IRP_MJ_PNP_POWER = *(PULONG)(ulAddress+1) + ulOffset;
