...
VOID Tcpview(HWND m_hWnd,ULONG ID,CClrListCtrl *m_list)
{
	...
	TCPUDPInfo = (PTCPUDPINFO)VirtualAlloc(0, (sizeof(TCPUDPINFO)+sizeof(TCPUDPINFO_INFORMATION))*1025,MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
	if (TCPUDPInfo)
	{
		memset(TCPUDPInfo,0,(sizeof(TCPUDPINFO)+sizeof(TCPUDPINFO_INFORMATION))*1025);
		ReadFile((HANDLE)LIST_TCPUDP,TCPUDPInfo,(sizeof(TCPUDPINFO)+sizeof(TCPUDPINFO_INFORMATION))*1025,&dwReadByte,0);
...
*/
		...
		VirtualFree(TCPUDPInfo,(sizeof(TCPUDPINFO)+sizeof(TCPUDPINFO_INFORMATION))*1025,MEM_RESERVE | MEM_COMMIT);
	}
	WCHAR lpwzTextOut[100];
	memset(lpwzTextOut,0,sizeof(lpwzTextOut));
