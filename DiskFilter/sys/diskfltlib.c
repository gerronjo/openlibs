...

{
	...
	g_cdo = on_diskperf_driver_entry(
...

{
    ...
    //
	// WindowsXPsp1编译环境下的驱动程序不需要（也不能够）在变量前面加*号(e.g.   KeNumberProcessors)。   
...
--*/
{
	...
	on_diskperf_remove_disk(deviceExtension->TargetDeviceObject, &deviceExtension->PhysicalDeviceName);
...

{
	...
	NTSTATUS mystatus;
	// add by tanwen 
	if(DeviceObject == g_cdo && on_diskperf_dispatch(DeviceObject,Irp,&mystatus))
		return mystatus;
...
--*/
{
	...
	on_diskperf_driver_unload(DriverObject);
