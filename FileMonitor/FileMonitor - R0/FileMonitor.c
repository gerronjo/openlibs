...
#pragma alloc_text(PAGE, SfDispatch )
...
#   define DEVOBJ_LIST_SIZE 64
	...
	KdPrint(("IoUnregisterFsRegistrationChange"));
    //
	...
	{
		PMYDATASTRUCT a;
		PLIST_ENTRY pEntry = RemoveHeadList(&g_List);
		a = CONTAINING_RECORD(pEntry, MYDATASTRUCT, ListEntry);
		ExFreePool(a);
	}
...
//	KdPrint(("SFilter!SfDisplayCreateFileName: Opened %08x:%08x %wZ\n", 
//                      Irp->IoStatus.Status,
//                      Irp->IoStatus.Information,
//                      name));
    //
...
// 		!NT_SUCCESS(Irp->IoStatus.Status));
	...
	{
	//	KdPrint(("!NT_SUCCESS(Irp->IoStatus.Status)\n"));
	...
	if(Irp->IoStatus.Information == FILE_DELETE_CHILD) 
	{
		KdPrint(("删除文件"));
	}
	...
	if(Irp->IoStatus.Information == FILE_CREATED) 
	{
		...
		{
			PMYDATASTRUCT mystruct = (PMYDATASTRUCT)ExAllocatePool(PagedPool, sizeof(MYDATASTRUCT));				
			RtlUnicodeStringToAnsiString(&aFilePath, &ObjectNameInformation->Name, TRUE);
			memcpy(mystruct->FilePath, aFilePath.Buffer, aFilePath.Length);
			mystruct->FilePath[aFilePath.Length] = 0;
			KdPrint(("创建文件成功 : %s", mystruct->FilePath));

			if(IsListEmpty(&g_List))
			{
				InsertTailList(&g_List, &mystruct->ListEntry);	
				KeSetEvent( pAppEvent, IO_NO_INCREMENT, FALSE);
			}
			else
			{
				InsertTailList(&g_List, &mystruct->ListEntry);	
...
NTSTATUS
SfFsControlMountVolumeComplete (
    IN PDEVICE_OBJECT DeviceObject, //要Attach的设备
    IN PIRP Irp,
    IN PDEVICE_OBJECT NewDeviceObject //过滤设备
    )
/*++
...
--*/
{
    ...
    SF_LOG_PRINT( SFDEBUG_DISPLAY_ATTACHMENT_NAMES,
		...
		KdPrint(("Attaching to file system   %p \"%wZ\" (%s)\n",
                   DeviceObject,
                   &devExt->DeviceName,
                   GET_DEVICE_TYPE_NAME(newDeviceObject->DeviceType)));
    //
...
{        
    ...
    for (i=0; i < 8; i++) {
        ...
        if (NT_SUCCESS(status)) {
			...
			KdPrint(("Attaching to volume        %p \"%wZ\"\n", 
                           newDevExt->AttachedToDeviceObject,
                           &newDevExt->DeviceName));
...
--*/
{
    ...
    switch (IoControlCode) {
		...
		case IOCTL_FILEMON_GETINFO:
			{
 //				KdPrint(("IOCTL_FILEMON_GETINFO\n"));

				PLIST_ENTRY pList = NULL;
				PMYDATASTRUCT pStruct = NULL;

				if (IsListEmpty(&g_List))
				{
					KeWaitForSingleObject(pAppEvent, Executive, KernelMode, FALSE, NULL);
				}
				pList = RemoveHeadList(&g_List);
				pStruct = CONTAINING_RECORD(pList, MYDATASTRUCT, ListEntry);					
				RtlCopyMemory(OutputBuffer, pStruct->FilePath, strlen(pStruct->FilePath)+1);

				IoStatus->Status = STATUS_SUCCESS;
			}
			break;

		case IOCTL_FILEMON_SETEVENT:
			{
				HANDLE hEvent = *(PHANDLE)InputBuffer;
				KdPrint(("IOCTL_FILEMON_REGAPPEVENT!\n"));
				status = ObReferenceObjectByHandle(hEvent, EVENT_MODIFY_STATE, 
					*ExEventObjectType, KernelMode, (PVOID*)&pAppEvent, NULL);						
		...
		case IOCTRL_STARTMON:
			{
				KdPrint(("IOCTRL_STARTMON\n"));
				bPass = FALSE;
				IoStatus->Status = STATUS_SUCCESS;
			}
			break;
